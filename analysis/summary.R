...
donors <- dbGetQuery(con, paste("SELECT ",
                                ...
                                " USING (donor_id) ",
                                " WHERE date_recieved >= '2010-07-01' AND ",
                                " date_recieved <= '2011-06-30' ",
...
"Smith", "Cappleman", "Sposato", "Cochran", "Waguespack", "Lane", # "Harris",
"Jackson", "Foulkes", "Mary O'Connor", "Burns", "Moreno",
...
levels(council_donors$recipient_id) <- last_names

totals <- aggregate(council_donors$totals,
                    list(council_donors$recipient_id),
                    sum)
...
for (alderman in last_names) {
  alderman_hist <- hist(council_donors[council_donors$recipient_id == alderman,
                                       'totals'],
                        breaks=seq(0,500000,100))
  write.table(data.frame(alderman,
                         paste(alderman_hist$counts[0:50], collapse=",")),
              "donation_counts.csv",
              append = TRUE,
              row.names = FALSE,
              col.names = FALSE)
